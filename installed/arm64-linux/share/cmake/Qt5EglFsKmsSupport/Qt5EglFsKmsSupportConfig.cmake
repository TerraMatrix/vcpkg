...
set(Qt5EglFsKmsSupport_VERSION_STRING 5.15.17)
...
function(_qt5_EglFsKmsSupport_process_prl_file prl_file_location Configuration lib_deps link_flags)
    ...
    if("${Configuration}" STREQUAL "DEBUG")
        set(_qt5_install_libs "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/")
    else()
...
macro(_populate_EglFsKmsSupport_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    if("${Configuration}" STREQUAL "DEBUG") # 1
        set(imported_location "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/${LIB_LOCATION}")
    else()
    ...
    if(_static_deps)
        ...
        #message(STATUS "Target Qt5::EglFsKmsSupport static links: ${_static_dep} through ${_static_dep_genex}") # Added for debugging
    endif()
    ...
    if(_static_link_flags)
        ...
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            ...
            #message(STATUS "Target Qt5::EglFsKmsSupport static link flags: ${_static_link_flags} through ${_static_link_flags_genex}")
        endif()
    endif()
...
if (NOT TARGET Qt5::EglFsKmsSupport)
    ...
    foreach(_module_dep ${_Qt5EglFsKmsSupport_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.17 ${_Qt5EglFsKmsSupport_FIND_VERSION_EXACT}
    ...
    if(EXISTS "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/libQt5EglFsKmsSupport.prl")
        _qt5_EglFsKmsSupport_process_prl_file(
            "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/libQt5EglFsKmsSupport.prl" DEBUG
            _Qt5EglFsKmsSupport_STATIC_DEBUG_LIB_DEPENDENCIES
            _Qt5EglFsKmsSupport_STATIC_DEBUG_LINK_FLAGS
        )
    endif()

    if(EXISTS "${_qt5EglFsKmsSupport_install_prefix}/lib/libQt5EglFsKmsSupport.prl")
        _qt5_EglFsKmsSupport_process_prl_file(
    ...
    if (EXISTS
        "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/libQt5EglFsKmsSupport.a"
      AND EXISTS
        "${_qt5EglFsKmsSupport_install_prefix}/debug/lib/" )
        _populate_EglFsKmsSupport_target_properties(DEBUG "libQt5EglFsKmsSupport.a" "" FALSE)
    endif()
